...
- Аппаратное ускорение корреляции с помощью CUDA (полноценный функционал в процессе разработки)
...
Optional arguments:
  ...
  -h, --help              Show this help message.
Example:
  ./corr_sig ./corr_sig -f test_iq.bin --prn 5 --tint 10 -o test.dat --iq --method=3 
...
Example:
  ./acq_sig -f test_iq.bin --tint=10 --iq --method 3 --threshold 35 
```
...
1. **Временная область (метод 1)** — последовательная корреляция полным перебором
2. **Параллельно по частоте (метод 2)** — на основе БПФ, параллелизм по ячейкам Доплера
3. **Параллельно по коду (метод 3)** — на основе БПФ, параллелизм по кодовым фазам (по умолчанию, самый быстрый)
...
В настоящий момент реализована функция, оборачивающая функцию-ядро для перемножения двух комплекснозначных массивов. Её использование возможно, но в данный момент закомментировано в `src/correlator/correlator.c` в функции `corr_parallel_code`. Это, по сути, заглушка для отладки сборки проекта совместно с CUDA

Планируется портирование на GPU всего `dsp/mixer.c` и реализация полноценных функций корреляции сигнала на GPU. Также планируется унификация интерфейсов работы с использованием трёх режимов: device=CPU, device=GPU, device=Jetson (особенность последнего в единой для CPU и GPU ОЗУ, что исключает узкое место пересылки данных)
